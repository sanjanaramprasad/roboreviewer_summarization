...
from BartForDataToTextGeneration_loop import BartForDataToText
import math
import random
import re
import argparse
...
def make_data(tokenizer, data_type = 'robo', path = '/home/sanjana'):
    if data_type == 'robo':
        train_file = path + '/roboreviewer_summarization/data/robo_train_linearized_per_study.csv'
        dev_file = path + '/roboreviewer_summarization/data/robo_dev_linearized_per_study.csv'
        test_file = path + '/roboreviewer_summarization/data/robo_test_linearized_per_study.csv'
...
def main():
    ...
    checkpoint = ModelCheckpoint('checkpoint_files_final/3e-5_bartcond_loop/',
    ...
    trainer.save_checkpoint("robo_model_epoch%s_adam_%s_bart_loop.ckpt"%(str(learning_rate), str(max_epochs)))
